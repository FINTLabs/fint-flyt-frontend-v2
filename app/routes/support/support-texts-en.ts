export const FLYT_DESCRIPTION_EN =
    'Fint Flyt is an integrations platform developed to simplify and reduce the number of integrations between systems. ' +
...
export const FAQ_EN: { header: string; content: string }[] = [
    {
        ...
        content:
            "Metadata can be used in all custom fields. In outgoing data, it's all the text fields, and additionally, the dropdown menus where you can select 'dynamic value.' This allows the user to build titles, personal information, or other fields with information from submitted data.",
    },
    {
        header: 'What do the symbols mean when I use metadata?',
        content:
            "The custom fields allow both free text and using data from the form. For a computer to distinguish free text from metadata, metadata appears in this format: '$if{metadata-id}'. This way, the system can recognize these references and replace the content with data from the submitted types. Therefore, it's important to maintain the format of metadata references when editing or adding free text in a custom field.",
    },
    {
        header: 'What is value conversion, and how do I use it?',
        content:
            'Value conversion is a field that allows you to use a defined rule to convert each occurrence of a value. Value conversions are set up in the value conversion menu and used similarly to metadata. Drag in the value conversion you want to use, followed by the value to be converted.',
    },
    {
        ...
        content:
            "Flyt will only have the ability to transform data based on rules set up in the configuration. Flyt isn't connected to the destination with information about what is accepted/not accepted. Flyt receives data, transforms it according to the given configuration, and forwards it. If the destination rejects the submission, an error message will be displayed in the types overview.",
    },
];
